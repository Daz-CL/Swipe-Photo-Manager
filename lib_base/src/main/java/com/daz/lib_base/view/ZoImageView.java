 ...
 */
public class ZoImageView extends AppCompatImageView {
    ...
    public boolean canScrollHorizontally(int direction) {
        if (mPinchMode == ZoImageView.PINCH_MODE_SCALE) {
            return true;
        }
    ...
    public interface OuterMatrixChangedListener {
         ...
         */
        void onOuterMatrixChanged(ZoImageView pinchImageView);
    }
    ...
    protected float calculateNextScale(float innerScale, float outerScale) {
        ...
        return currentScale < MAX_SCALE? MAX_SCALE : innerScale;
        /*float currentScale = innerScale * outerScale;
    ...
    public ZoImageView(Context context) {
        super(context);
        initView();
    }

    public ZoImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public ZoImageView(Context context, AttributeSet attrs, int defStyle) {
     ...
     * 在onTouchEvent末尾被执行.
     */
    private GestureDetector mGestureDetector = new GestureDetector(ZoImageView.this.getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (mOnLongClickListener != null) {
                mOnLongClickListener.onLongClick(ZoImageView.this);
            }
        }
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            if (mOnClickListener != null) {
                mOnClickListener.onClick(ZoImageView.this);
            }
            return true;
        }
    });
    ...
    private void scaleEnd() {
        ...
        // 第二层缩放比例
        float outerScale = MathUtils.getMatrixScale(mOuterMatrix)[0];
        // 控件大小（虽然不再限制，但这里仍保留获取，可能在其他逻辑中有需要）
        float displayWidth = getWidth();
        float displayHeight = getHeight();
        // 最大缩放比例（不再用于限制缩小情况）
        float maxScale = getMaxScale();
        // 比例修正（只考虑放大超过最大值的修正情况）
        float scalePost = 1f;
